    ...
    return (
      ...
      >
        <div
          ref={trackRef}
          className={cn(
            "relative rounded-full cursor-pointer select-none transition-all duration-300 ease-out",
            trackSizes[size],
            variants[variant].track,
            orientation === "horizontal" ? "w-full" : "h-full",
            disabled && "cursor-not-allowed",
            hovered && !disabled && !variant.startsWith("neon-") && "shadow-lg",
          )}
          style={trackStyle}
        ...
        >
          ...
          <motion.div
            ref={thumbRef}
            className={cn(
              "absolute rounded-full cursor-grab active:cursor-grabbing transition-all duration-300 ease-out select-none",
              thumbSizes[size],
              isDragging
                ? variants[variant].thumbActive || variants[variant].thumbHover
                : hovered
